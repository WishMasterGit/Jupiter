...
fn convolve_rows(data: &Array2<f32>, kernel: &[f32]) -> Array2<f32> {
    ...
    if h * w >= PARALLEL_PIXEL_THRESHOLD {
        convolve_rows_parallel(data, kernel, h, w)
    } else {
        convolve_rows_sequential(data, kernel, h, w)
    }
}

fn convolve_rows_parallel(data: &Array2<f32>, kernel: &[f32], h: usize, w: usize) -> Array2<f32> {
...
fn convolve_cols(data: &Array2<f32>, kernel: &[f32]) -> Array2<f32> {
    let (h, w) = data.dim();
    if h * w >= PARALLEL_PIXEL_THRESHOLD {
        convolve_cols_parallel(data, kernel, h, w)
    } else {
        convolve_cols_sequential(data, kernel, h, w)
    }
}

fn convolve_cols_parallel(data: &Array2<f32>, kernel: &[f32], h: usize, w: usize) -> Array2<f32> {
    let radius = kernel.len() / 2;
    let rows: Vec<Vec<f32>> = (0..h)
        .into_par_iter()
        .map(|row| {
